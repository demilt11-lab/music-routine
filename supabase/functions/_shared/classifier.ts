// FLOWSTATE biometric state classifier — pure logic, no Deno/runtime deps.
// Single source of truth for activity profiles and state classification,
// imported by the state-classifier and playlist-engine edge functions and
// unit-tested from src/test/ via vitest.
...
// UI activity names (activity_types table) → classifier profile keys.
// The UI taxonomy predates the classifier; map until they are unified.
export const UI_ACTIVITY_TO_PROFILE: Record<string, string> = {
...
export function resolveActivityProfile(activityType: string): ActivityProfile {
  return (
    ACTIVITY_PROFILES[activityType] ??
    ACTIVITY_PROFILES[UI_ACTIVITY_TO_PROFILE[activityType] ?? ""] ??
    ACTIVITY_PROFILES["study"]
  );
}
